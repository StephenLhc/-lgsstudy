// 聖經 66 卷資料（繁體中文書名）
// 每一卷以 [書名, 每章節數陣列] 表示：
//   陣列長度＝該卷章數；verses[章 - 1]＝該章實際節數。
// 資料依新教通用聖經（和合本）章節編排。

type BookEntry = [name: string, verses: number[]];
...
const OLD_TESTAMENT: BookEntry[] = [
  ...
  ['民數記', [54,34,51,49,31,27,89,26,23,36,35,16,33,45,41,50,13,32,22,29,35,41,30,25,18,22,23,35,40,16,54,42,36,29,34,13]],
  ...
  ['歷代志上', [54,55,24,43,26,81,40,40,44,14,47,40,14,17,29,43,27,17,19,8,30,19,32,31,31,39,34,21,30]],
  ...
  ['以賽亞書', [31,22,26,6,30,13,25,22,21,34,16,6,22,32,9,14,14,7,25,6,17,25,18,23,12,21,13,29,24,33,9,20,24,17,10,22,38,22,8,31,29,25,28,28,25,13,15,22,26,11,23,15,12,17,13,12,21,14,21,22,11,12,19,12,25,24]],
  ...
  ['以西結書', [28,10,27,17,17,14,27,18,11,22,25,28,23,23,8,63,24,32,14,49,32,31,40,27,17,21,36,26,21,26,18,32,33,31,15,38,28,23,29,49,26,20,27,31,25,24,23,35]],
  ['但以理書', [21,49,30,37,31,28,28,27,27,21,45,13]],
...
const NEW_TESTAMENT: BookEntry[] = [
  ...
  ['提多書', [16,15,15]],
  ['腓利門書', [25]],
  ['希伯來書', [14,18,19,16,14,20,28,13,28,39,40,29,25]],
  ['雅各書', [27,26,18,17,20]],
  ['彼得前書', [25,25,22,19,14]],
  ['彼得後書', [21,22,18]],
  ['約翰一書', [10,29,24,21,21]],
  ...
  ['約翰三書', [14]],
  ['猶大書', [25]],
  ['啟示錄', [20,29,22,11,14,17,17,13,21,11,19,17,18,20,8,21,18,24,21,15,27,21]],
];
...
export const BIBLE_BOOKS: BibleBook[] = [
  ...OLD_TESTAMENT.map(([name, verses]) => ({ name, testament: 'old' as const, chapters: verses.length, verses })),
  ...NEW_TESTAMENT.map(([name, verses]) => ({ name, testament: 'new' as const, chapters: verses.length, verses })),
];
...
export function getChapterCount(bookName: string): number {
  return BIBLE_BOOKS.find((b) => b.name === bookName)?.chapters ?? 0;
}

// 取得某書卷某章的節數（章別超出範圍回 0）
export function getVerseCount(bookName: string, chapter: number): number {
  const book = BIBLE_BOOKS.find((b) => b.name === bookName);
  if (!book || chapter < 1 || chapter > book.verses.length) return 0;
  return book.verses[chapter - 1];
}

// 66 卷英文縮寫，順序與 BIBLE_BOOKS 一一對應
// 採用香港聖經公會 rcuv.hkbs.org.hk 網站使用的 bibleshort 代碼
...
const BOOK_CODE_MAP: Record<string, string> = Object.fromEntries(
  BIBLE_BOOKS.map((b, i) => [b.name, BIBLE_BOOK_CODES[i]]),
);

// 取得繁體書名對應的英文縮寫（找不到回 null）
export function getBookCode(bookName: string): string | null {
  return BOOK_CODE_MAP[bookName.trim()] ?? null;
}
